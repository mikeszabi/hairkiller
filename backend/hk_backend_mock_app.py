...
_calibration_points: list[list[list[int]]] = []
...
def frame_snapshot():
    return StreamingResponse(iter([_jpeg_bytes()]), media_type="image/jpeg")


@app.get("/camera/settings")
def get_camera_settings():
    return {"ok": True, "settings": _camera_settings}
...
def set_camera_settings(
    ...
    exposure: int | None = Query(default=None),
    white_balance: int | None = Query(default=None),
    fps: float | None = Query(default=None),
):
    if auto_exposure is not None:
        _camera_settings["auto_exposure"] = 3 if auto_exposure else 1
    if auto_wb is not None:
        _camera_settings["auto_wb"] = 1 if auto_wb else 0
    if exposure is not None:
        _camera_settings["exposure"] = exposure
    if white_balance is not None:
        _camera_settings["white_balance"] = white_balance
    if fps is not None:
        _camera_settings["fps"] = fps
    return {"ok": True, "settings": _camera_settings}
...
def latency_benchmark(samples: int = Query(default=20, ge=5, le=200)):
    return {
        "ok": True,
        "samples": samples,
        "read_wait_ms": {"min": 0.4, "avg": 0.8, "max": 1.2},
        "frame_age_ms": {"min": 10.0, "avg": 12.0, "max": 16.0},
        "encode_ms": {"min": 0.8, "avg": 1.2, "max": 1.8},
        "total_pipeline_ms": {"min": 1.4, "avg": 2.0, "max": 3.0},
        "camera_stats": stats()["camera"],
        "settings": _camera_settings,
        "mock": True,
    }
...
def toggle_calibration_detection(enabled: bool = Query(...)):
    ...
    return {"detection_enabled": enabled, "red_dot": _red_dot_enabled}
...
def set_detection_hsv(
    lower1_h: int = Query(...), lower1_s: int = Query(...), lower1_v: int = Query(...),
    upper1_h: int = Query(...), upper1_s: int = Query(...), upper1_v: int = Query(...),
    lower2_h: int = Query(...), lower2_s: int = Query(...), lower2_v: int = Query(...),
    upper2_h: int = Query(...), upper2_s: int = Query(...), upper2_v: int = Query(...),
):
    global _hsv_lower1, _hsv_upper1, _hsv_lower2, _hsv_upper2
    _hsv_lower1 = [lower1_h, lower1_s, lower1_v]
    _hsv_upper1 = [upper1_h, upper1_s, upper1_v]
    _hsv_lower2 = [lower2_h, lower2_s, lower2_v]
    _hsv_upper2 = [upper2_h, upper2_s, upper2_v]
...
def read_frame_hsv(x: int = Query(...), y: int = Query(...)):
    return {"x": x, "y": y, "hsv": [0, 180, 255], "bgr": [10, 10, 255], "rgb": [255, 10, 10], "mock": True}
...
def store_calibration_point():
    image_point = _mock_points()[len(_calibration_points) % len(_mock_points())]
    _calibration_points.append([image_point, _mover_pos[:]])
...
def save_calibration():
    ...
    return {"status": "saved", "count": len(_calibration_points), "mock": True}
